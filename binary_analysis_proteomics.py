#       Proteomics-Data usage example of Appearance Differential Expression analysis functions
#   for article "Differential expression analysis of binary appearance patterns"
#       Dataset is from: https://www.medrxiv.org/content/10.1101/2022.12.22.22283845v1.full
#
...
FL_PREFIX = 'proteomics/proteomics.de_vs_binary'
...
if __name__ == '__main__':
    ...
    prot_freqs = (app_results_df[f'BCC count']+app_results_df[f'SCC count']) / float(len(ypos))
    for perc_c in [0.9, 0.5, 0.1]:
        n_of_prots = np.count_nonzero(prot_freqs<perc_c)
        perc_of_samples = n_of_prots / len(prot_freqs)
        print(f" {n_of_prots}, i.e. {100*perc_of_samples:.1f}% of {len(prot_freqs)} proteins appear in less than {perc_c*100:.0f}% of samples")
    ...
    app_results_df[['Wilcoxon STAT', 'Wilcoxon Pvalue']] = app_results_df.apply(au.Wilcoxon_test, axis=1)
